...
%% ---------------- USER INPUTS ----------------
csvFile = "ClarkY.csv";      % columns: x_over_c, y_over_c
chord = c*1000;                   % [mm]

T_w = T(1)*1000; % in N*mm

% Cell boundaries (mm) => 2 cells (NO TE cell)
xCuts = [0.0 0.25 0.75]*chord;   % [mm]
...
sparStations = [0.25 0.75]*chord;     % [mm] (same as your tubeStations)
...
eq3 = 2*q1*A1 + 2*q2*A2 + sumGJ*theta1 == T_w;
...
title(sprintf('Wing Torsional Shear Flow (T = %.1f N-mm)', T_w));
