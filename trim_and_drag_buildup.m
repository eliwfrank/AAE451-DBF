...
S_wet_w = wing_area_total * (1 + t_c)*2; %wetted area of wing
Q_w = 1;
...
S_wet_ht = tail_area_h * 2 * (1 + t_c_tail); %wetted area of horizontal tail

FF_vt = (1 + 0.6/(x_c_max)*(t_c_tail) + 100*(t_c_tail)^4) * (1.34*M^0.18*(cos(Lambda_m_vtail))^0.28); %vertical tail form factor
S_wet_vt = S_vt * 2 * (1 + t_c_tail); %wetted area of vertical tail
...
CD_0 = (C_D_0_f + C_D_0_w + C_D_0_ht + C_D_0_vt + C_D_0_lg_main + C_D_0_lg_nose)*1.1; 
fprintf("\nDRAG BUILD-UP ------------------------\n")
